...
from llm.prompts import (
    ...
    validate_player_action_prompt, revise_storyline_prompt, revise_options_prompt, inform_invalid_action_prompt,
)
import traceback
...
async def generate_gm_response(
    ...
    try:
        ...
        if not saved_game:
            ...
            return {"dm_response": "Error: Unable to find the saved game session. Please start a new game.",
                    "full_storyline": ""}
        ...
        if is_new_campaign:
            ...
            if feedback:
                revise_prompt_content = revise_storyline_prompt(context, dm_response_text, feedback)
                revise_msg = [{"content": revise_prompt_content, "role": "system"}]
                revised_response = await get_agent_response(dm_agent, "DMAgent", revise_msg, ["dm_response"])
                dm_response_text = revised_response.get("dm_response", "") if isinstance(revised_response, dict) else ""

            # 4. Validate options in DM response
            options_validation_prompt_content = validate_options_prompt(context, dm_response_text)
            options_validation_msg = [{"content": options_validation_prompt_content, "role": "user"}]
            options_feedback_response = await get_agent_response(storyteller_agent, "StorytellerAgent", options_validation_msg, ["feedback"])
            options_feedback = options_feedback_response.get("feedback", "") if isinstance(options_feedback_response, dict) else ""

            # 5. Revise options based on feedback if needed
            if options_feedback:
                revise_options_prompt_content = revise_options_prompt(context, dm_response_text, options_feedback)
                revise_options_msg = [{"content": revise_options_prompt_content, "role": "system"}]
                revised_options_response = await get_agent_response(dm_agent, "DMAgent", revise_options_msg, ["dm_response"])
                dm_response_text = revised_options_response.get("dm_response", "") if isinstance(revised_options_response, dict) else ""
            ...
            action_validation_prompt_content = validate_player_action_prompt(context, storyline, user_input)
            action_validation_msg = [{"content": action_validation_prompt_content, "role": "user"}]
            storyteller_agent = agents.get("StorytellerAgent")
            if not storyteller_agent:
                return {"dm_response": "Error validating the player's action.", "full_storyline": storyline}
            action_feedback_response = await get_agent_response(storyteller_agent, "StorytellerAgent", action_validation_msg, ["feedback"])
            action_feedback = action_feedback_response.get("feedback", "") if isinstance(action_feedback_response, dict) else ""

            if action_feedback:
                # 2. Inform the player if action is invalid
                inform_feedback_prompt_content = inform_invalid_action_prompt(context, storyline, user_input, action_feedback)
                inform_feedback_msg = [{"content": inform_feedback_prompt_content, "role": "system"}]
                inform_feedback_response = await get_agent_response(dm_agent, "DMAgent", inform_feedback_msg, ["dm_response"])
                dm_response_text = inform_feedback_response.get("dm_response", "") if isinstance(inform_feedback_response, dict) else ""
                return {"dm_response": dm_response_text, "full_storyline": storyline}
            ...
            storyline_validation_prompt_content = validate_storyline_prompt(context, dm_response_text)
            storyline_validation_msg = [{"content": storyline_validation_prompt_content, "role": "system"}]
            storyline_feedback_response = await get_agent_response(storyteller_agent, "StorytellerAgent", storyline_validation_msg, ["feedback"])
            storyline_feedback = storyline_feedback_response.get("feedback", "") if isinstance(storyline_feedback_response, dict) else ""

            # 5. Revise storyline based on feedback if needed
            if storyline_feedback:
                revise_storyline_prompt_content = revise_storyline_prompt(context, dm_response_text, storyline_feedback)
                revise_storyline_msg = [{"content": revise_storyline_prompt_content, "role": "system"}]
                revised_storyline_response = await get_agent_response(dm_agent, "DMAgent", revise_storyline_msg, ["dm_response"])
                dm_response_text = revised_storyline_response.get("dm_response", "") if isinstance(revised_storyline_response, dict) else ""
